...
# For mEPSP
def rs_compensation(sweep, fs, sweep_start=0.01, sweep_duration=0.01):
    ...
    rs = np.abs(pulse) * 10**3 / np.min(np.abs(segment))
    v_cmd = -70

    corrected_sweep = np.zeros_like(sweep)

    for i, i_rec in enumerate(sweep):
        vm = v_cmd - i_rec * rs * 10**(-3)
        corrected_sweep[i] = i_rec * (v_cmd / vm)

    return corrected_sweep


def mEPSC_detect_events(sweep_data, fs, sweep_start, sweep_duration):
    ...
    for sweep in sweep_data:
        ...
        sweep = rs_compensation(sweep, fs)
        segment = sweep[sweep_start_idx:sweep_end_idx]
        ...
        if deviation > drift_threshold:
            ...
            for idx in peaks:
                ...
                if (np.all(dvdt[idx-int(decay_time_threshold/dt_ms):idx] < 0) and 
...
def mEPSC_event_annotation(sweep_data, event_times_all, fs, sweep_duration):
    ...
    for i in range(len(sweep_data)):
        ...
        event_values = [filtered[idx]/25 + offset for idx in event_indices]

        ax.plot(time, filtered/25 + offset, color='black', linewidth=0.1)
        ax.plot(time, baseline/25 + offset, color='gray', linewidth=0.4)
...
def mEPSC_cdf(event_amps_all_condition, n_bins=50):
    ...
    for condition, event_amps_summary in event_amps_all_condition.items():
        ...
        for neuron, event_amps in event_amps_summary.items():
            values.extend(np.abs(event_amps))
...
for condition in conditions:
    ...
    for mouse_id, slice_num in zip(mouse_ids, slice_nums):
        ...
        sweep_data, event_counts, event_times_all, event_amps = mEPSC_detect_events(sweep_data, fs, sweep_start=0.4, sweep_duration=10.0)

        if sweep_data is not None:

            fig_event_annotation = mEPSC_event_annotation(sweep_data, event_times_all, fs, sweep_duration=10.0)
...
cdf_fig, cdf_stats = mEPSC_cdf(event_amps_all_conditions)
...
cdf_fig.savefig(f'{save_path}/mEPSC_amp_distribution.png', dpi=1000)
average_amp_fig.savefig(f'{save_path}/mEPSC_average_amp.png', dpi=1000)
freq_fig.savefig(f'{save_path}/mEPSC_frequency.png', dpi=1000)

cdf_stats.to_csv(f'{save_path}/mEPSC_amp_distribution_stats.csv', index=False)
average_amp_stats.to_csv(f'{save_path}/mEPSC_average_amp_stats.csv', index=False)
freq_stats.to_csv(f'{save_path}/mEPSC_frequency_stats.csv')        
